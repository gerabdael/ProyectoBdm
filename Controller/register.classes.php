...
class Register extends Dbh{
    ...
    protected function register($completeName,$email,$password, $user,$telephone){
        $stmt = $this->connect()->prepare('INSERT INTO USUARIO(nombreCompleto,email,contraseña,alias,diaregistro,telefono,activo,tipoU,StatusU,creadopor) VALUES(?,?,?,?,sysdate(),?,true,2,"A",117);');
        $hashPwd = password_hash($password,PASSWORD_DEFAULT);
        if (!$stmt-> execute(array($completeName,$email,$hashPwd,$user,$telephone))) {
            $stmt = null;
            header("location: ../Views/Register.php?error=stmtfaile");
            exit();
        }
        $stmt = null;
    }
